...
async function postInstagram({ accessToken, igUserId, caption, mediaUrl }:{
  accessToken: string; igUserId: string; caption: string; mediaUrl: string;
}) {
  ...
  if (!cData?.id) throw new Error(`Failed to create Instagram media container: ${JSON.stringify(cData)}`);
  ...
  if (!pData?.id) throw new Error(`Failed to publish Instagram media: ${JSON.stringify(pData)}`);
...
export const socialPoster = onCall(async (req) => {
  if (!req.auth) throw new Error("Unauthenticated");
  const { uid } = req.auth;

  const { platform, caption, mediaUrl, scheduled_at = null, extra = {} } = req.data || {};
  if (!platform || !mediaUrl) throw new Error("Missing platform or mediaUrl");
  ...
  try {
    if (platform === "instagram") {
      ...
      const igUserId = (extra as any).igUserId; // The user's Instagram User ID is required
      if (!igUserId) throw new Error("Instagram User ID (`igUserId`) is required in `extra` options.");
...
export const schedulePublisher = onCall(async () => {
  ...
  const publishPromises = snap.docs.map(async (doc) => {
    ...
    try {
      if (post.platform === "instagram") {
        ...
        if (!igUserId) throw new Error("Instagram User ID is missing from scheduled post.");
