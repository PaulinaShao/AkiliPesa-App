...
exports.tickCalls = exports.endCall = exports.getAgoraToken = exports.resetTrialCredits = exports.seeddemo = exports.buyPlan = exports.onpostcreate = exports.onusercreate = exports.onFeedbackCreated = exports.onDeliveryProofAdded = exports.onProductCreatedVerify = exports.onRefundRequest = exports.onOrderStatusChange = exports.verifyAndReleaseEscrow = exports.createEscrowOnOrder = exports.redeemReward = exports.onAIInteractionReward = exports.onReferralReward = exports.onProductSaleReward = exports.updateAgentRanks = exports.aggregateDailyRevenue = exports.onWithdrawalApproved = exports.onVoiceUpload = exports.realtimePayoutManager = exports.enforceTransactionUid = void 0;
...
const enforceTransactionUid_1 = require("./enforceTransactionUid");
...
const realtimePayoutManager_1 = require("./realtimePayoutManager");
...
const openvoiceTrigger_1 = require("./openvoiceTrigger");
...
exports.onWithdrawalApproved = (0, firestore_1.onDocumentUpdated)("withdrawalRequests/{reqId}", async (event) => {
    if (!event.data) {
        return;
    }
    ...
    if (before.status !== "approved" && after.status === "approved") {
        ...
        try {
            const res = await (0, node_fetch_1.default)(MAKE_WEBHOOK_URL, {
                method: "POST",
...
exports.aggregateDailyRevenue = (0, scheduler_1.onSchedule)("every day 00:00", async () => {
...
exports.updateAgentRanks = (0, scheduler_1.onSchedule)("every day 02:00", async () => {
...
exports.onProductSaleReward = (0, firestore_1.onDocumentCreated)("sales/{saleId}", async (event) => {
    var _a;
    const sale = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
    if (!sale)
        return;
...
exports.onReferralReward = (0, firestore_1.onDocumentCreated)("referrals/{refId}", async (event) => {
    var _a;
    const ref = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
...
exports.onAIInteractionReward = (0, firestore_1.onDocumentUpdated)("aiSessions/{id}", async (event) => {
    var _a, _b;
    const before = (_a = event.data) === null || _a === void 0 ? void 0 : _a.before.data();
    const after = (_b = event.data) === null || _b === void 0 ? void 0 : _b.after.data();
    // Trigger when session ends
    if ((before === null || before === void 0 ? void 0 : before.isActive) === true && (after === null || after === void 0 ? void 0 : after.isActive) === false && after.duration > 60) {
...
exports.verifyAndReleaseEscrow = (0, firestore_1.onDocumentUpdated)("deliveryProofs/{id}", async (event) => {
    if (!event.data)
        return;
    const before = event.data.before.data();
    const after = event.data.after.data();
...
// 4. Refund Handling
exports.onRefundRequest = (0, firestore_1.onDocumentCreated)("refunds/{id}", async (event) => {
    var _a;
...
exports.onProductCreatedVerify = (0, firestore_1.onDocumentCreated)("products/{productId}", async (event) => {
    var _a;
...
exports.onDeliveryProofAdded = (0, firestore_1.onDocumentCreated)("deliveryProofs/{id}", async (event) => {
    var _a;
    const proof = (_a = event.data) === null || _a === void 0 ? void 0 : _a.data();
    if (!proof || !proof.url) {
 ...
 */
exports.onFeedbackCreated = (0, firestore_1.onDocumentCreated)("feedback/{id}", async (event) => {
    var _a;
...
exports.onusercreate = functions.auth.user().onCreate(async (user) => {
    ...
    batch.set(userRef, {
        uid,
        handle: (email === null || email === void 0 ? void 0 : email.split('@')[0]) || `user_${uid.substring(0, 5)}`,
...
exports.buyPlan = functions.https.onCall(async (data, context) => {
    ...
    await db.runTransaction(async (t) => {
        ...
        const wallet = ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.wallet) || { balance: 0, plan: {} };
        if (method === 'wallet') {
            ...
            t.update(userRef, {
                ...
                'wallet.plan': {
                    ...
                    expiry: admin.firestore.Timestamp.fromDate(new Date(Date.now() + plan.validityDays * 86400000)),
                },
            });
...
exports.resetTrialCredits = functions.pubsub
    ...
    snapshot.docs.forEach(doc => {
        var _a;
        const user = doc.data();
        const lastReset = (_a = user.wallet.lastTrialReset) === null || _a === void 0 ? void 0 : _a.toDate();
...
exports.getAgoraToken = functions.https.onCall(async (data, context) => {
    ...
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'Login required.');
    const uid = context.auth.uid;
    const { agentId, agentType, mode, channelName: requestedChannelName } = data;
    if (!agentId || !agentType || !mode)
    ...
    const wallet = userDoc.data().wallet || { balance: 0, credits: 0 };
    // Resolve agent + price
    let pricePerSecondCredits = 0.1;
    if (agentType === 'admin') {
        const a = await db.collection('adminAgents').doc(agentId).get();
        if (!a.exists)
            throw new functions.https.HttpsError('not-found', 'Admin agent not found');
        if (a.data().status !== 'active')
            throw new functions.https.HttpsError('failed-precondition', 'Agent inactive');
        pricePerSecondCredits = (_a = a.data().pricePerSecondCredits) !== null && _a !== void 0 ? _a : 0.1;
    }
    else {
    ...
    if (((_b = wallet.credits) !== null && _b !== void 0 ? _b : 0) < pricePerSecondCredits) {
    ...
    const appId = (_c = functions.config().agora) === null || _c === void 0 ? void 0 : _c.appid;
    const appCert = (_d = functions.config().agora) === null || _d === void 0 ? void 0 : _d.certificate;
    if (!appId || !appCert) {
        throw new functions.https.HttpsError('failed-precondition', 'Agora credentials are not configured.');
    }
    const expire = 3600;
    const token = agora_access_token_1.RtcTokenBuilder.buildTokenWithUid(appId, appCert, channelName, 0, agora_access_token_1.RtcRole.PUBLISHER, expire);
    // Log call session
...
exports.endCall = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'Login required.');
    const uid = context.auth.uid;
    const { callId, seconds } = data;
    if (!callId || typeof seconds !== 'number')
        throw new functions.https.HttpsError('invalid-argument', 'Missing fields');
    ...
    if (call.callerId !== uid)
        throw new functions.https.HttpsError('permission-denied', 'Only caller can end the call');
    ...
    await db.runTransaction(async (t) => {
        ...
        if (!u.exists)
            throw new functions.https.HttpsError('not-found', 'User profile not found for billing.');
        const wallet = u.data().wallet || { credits: 0 };
        const creditsToCharge = Math.ceil(seconds) * ((_a = call.pricePerSecondCredits) !== null && _a !== void 0 ? _a : 0.1);
        t.update(userRef, {
...
exports.tickCalls = functions.pubsub.schedule('every 2 minutes').onRun(async () => {
