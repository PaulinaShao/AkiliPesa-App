...
exports.onCallRoomWrite = exports.callBillingTick = void 0;
...
// ─────────────────────────────────────────────
exports.callBillingTick = (0, https_1.onRequest)({ region: "us-central1" }, async (req, res) => {
    if (req.method !== "POST") {
    ...
    try {
        const result = await db.runTransaction(async (tx) => {
            ...
            if (!callSnap.exists) {
                return { status: "not_found", chargedSeconds: 0 };
            }
            ...
            const lastBilledAt = call.lastBilledAt || startedAt;
            if (!lastBilledAt) {
            ...
            const elapsedSec = Math.floor((now.toMillis() - lastBilledAt.toMillis()) / 1000);
            if (elapsedSec <= 0) {
            ...
            const beforePlanCredits = wallet?.plan?.credits || 0;
            const beforeWalletTZS = wallet?.balanceTZS || 0;
            ...
            if (remaining > 0 && walletBalanceTZS > 0) {
                const walletCreditsAvailable = Math.floor(walletBalanceTZS / CREDIT_VALUE_TZS);
            ...
            if (remaining > 0) {
                ...
                const planWentLow = beforePlanCredits >= LOW_PLAN_CREDITS_THRESHOLD &&
                    afterPlanCredits < LOW_PLAN_CREDITS_THRESHOLD;
                const walletWentLow = beforeWalletTZS >= LOW_WALLET_TZS_THRESHOLD &&
                    afterWalletTZS < LOW_WALLET_TZS_THRESHOLD;
                const anyDepleted = (beforePlanCredits > 0 && afterPlanCredits === 0) ||
            ...
            tx.update(callRef, {
                ...
                totalCreditsRequired: (call.totalCreditsRequired || 0) + requiredCredits,
                endedAt: newStatus !== "active"
            ...
            tx.set(billingRef, {
                ...
                agentId,
                mode,
                audioSeconds: mode === "audio"
                    ? admin.firestore.FieldValue.increment(elapsedSec)
                    : admin.firestore.FieldValue.increment(0),
                videoSeconds: mode === "video"
                    ? admin.firestore.FieldValue.increment(elapsedSec)
                    : admin.firestore.FieldValue.increment(0),
                planCreditsUsed: planUsed > 0
                    ? admin.firestore.FieldValue.increment(planUsed)
                    : admin.firestore.FieldValue.increment(0),
                walletDebitTZS: walletCreditsUsed > 0
                    ? admin.firestore.FieldValue.increment(walletCreditsUsed * CREDIT_VALUE_TZS)
                    : admin.firestore.FieldValue.increment(0),
                lastTickAt: now,
            }, { merge: true });
            return {
...
// ─────────────────────────────────────────────
exports.onCallRoomWrite = (0, firestore_1.onDocumentWritten)({ region: "us-central1", document: "callRooms/{callId}" }, async (event) => {
    ...
    if (prevStatus === "active" &&
        ...
        const billingUid = billing.billingUid ||
            cur.billingUid ||
            cur.callerId ||
            cur.customerId;
        ...
        console.log(`✅ Finalized billing for call ${callId} (status: ${newStatus}) – gross: ${grossAmount}, platform: ${platformShareTZS}, agent: ${agentShareTZS}`);
    }
});
//# sourceMappingURL=realtimeBilling.js.map