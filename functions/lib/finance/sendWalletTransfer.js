 ...
 * Path: /wallets/{uid}
 */
export const sendWalletTransfer = onCall({ region: "us-central1" }, async (request) => {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        // update balances
        tx.set(senderRef, { balanceTZS: senderBalance - totalDebit, updatedAt: now }, { merge: true });
        tx.set(receiverRef, { balanceTZS: receiverBalance + numericAmount, updatedAt: now }, { merge: true });
        tx.set(platformRef, { balanceTZS: platformBalance + fee, updatedAt: now }, { merge: true });
        // log transactions
        tx.set(txCol.doc(), {
            uid: fromUid,
