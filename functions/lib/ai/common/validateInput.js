...
export function validateAIInput(mode, payload) {
    if (!mode) {
    ...
    if (["text", "image", "music", "video"].includes(mode) && !payload.prompt) {
        throw new HttpsError("invalid-argument", "'payload.prompt' is required for this mode.");
    }
    if (mode === "audio" && !payload.audioUrl && !payload.text) {
        throw new HttpsError("invalid-argument", "audio mode requires 'audioUrl' or 'text'.");
    }
}
