...
import type { VendorPayload } from "./adapters/types";
...
const VENDOR_MAP: Record<string, any> = {
  openai, pi,
...
export const aiRouter = onCall(async (req) => {
  ...
  const {
    requestType,              // "text|image|video|audio|music|sign|document|weather"
  ...
  if (!adapter?.run) throw new Error(`Adapter not found for vendor: ${chosen}`);
  ...
  // assemble payload
  const payload: VendorPayload = { input, options, uid, requestId };
