 ...
 */
const BASE_VENDOR_PROFILES: Record<string, {
  costPerUnit: number;   // estimated cost in USD per generation unit
  quality: number;       // 1-100
  latency: number;       // ms estimate
  supports: string[];    // content types supported
}> = {
...
export async function selectVendor({
  requestType,      // "text|video|audio|music|voice|sign|chat"
  ...
  if (plan === "free") candidates = candidates.slice(-3);     // cheaper vendors
  if (plan === "starter") candidates = candidates.slice(-5); // balanced
