...
export default function useAgoraConnection(channelName?: string | null): UseAgora {
  ...
  const leave = useCallback(async () => {
    ...
    const startTime = client?.getRTCStats().ConnectTime;
    const duration = startTime ? Math.floor((Date.now() - startTime) / 1000) : 0;

    if (channelRef.current) {
        await addDoc(collection(firestore, 'users', currentUser.uid, 'callHistory'), {
            channelName: channelRef.current,
            duration,
            endedAt: serverTimestamp(),
            // You might want to add who the call was with
        });
    }
