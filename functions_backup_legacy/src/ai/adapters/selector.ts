...
type VendorModule = {
  name: string;
  run: (p: VendorPayload) => Promise<VendorResult>;
  baseCost: number;
  supported: string[]; // ["text","image","video","audio","sign"]
};

const VENDORS: Record<string, VendorModule> = {
...
const PLAN_LIMITS: Record<string, { maxCost: number; pref: string[] }> = {
...
export function selectVendor(params: { requestType: string; plan: string; wallet: number }) {
...
export function estimateCost(v: VendorModule, _requestType: string, _options: any) {
  // For now use baseCost; Studio can later expand with duration/length multipliers
  return v.baseCost;
}

export function computeUserPrice(cost: number, plan: string) {
  // margin bands
...
export async function callVendor(vendor: VendorModule, payload: VendorPayload): Promise<VendorResult> {
  try {
