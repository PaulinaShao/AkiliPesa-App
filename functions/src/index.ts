...
import type { Change, DocumentSnapshot } from 'firebase-functions/v1';
...
export const onOrderUpdate = onDocumentUpdated(
  ...
  async (event: Change<DocumentSnapshot>) => {
    ...
    const platformShare = type === 'service' ? gross * 0.4 : 0;
    const ownerShareBase = type === 'service' ? gross * 0.5 : 0;
