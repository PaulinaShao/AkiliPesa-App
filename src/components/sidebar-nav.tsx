      ...
      <nav className="flex-1">
        <ul className="space-y-2">
          {mainNavLinks.map(({ href, icon: Icon, label }) => {
            ...
            else if (href === '/profile') isActive = pathname.startsWith('/profile');
            else if (href === '/create/ai') isActive = pathname.startsWith('/create');
            else isActive = pathname.startsWith(href);

            return (
              <li key={label}>
                <Link href={href}>
                  <Button variant={isActive ? 'secondary' : 'ghost'} className="w-full justify-start text-lg h-12 text-foreground">
