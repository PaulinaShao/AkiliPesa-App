...
/* ------------------------------------------
   MAIN PAGE (AGENT PUBLIC PROFILE)
------------------------------------------- */
export default function PublicAgentProfilePage() {
  ...
  // ✅ Graceful SSR fallback for uninitialized Firebase
  const firebase = useMemoFirebase();
  if (!firebase) {
    return (
  ...
  const { data: users, isLoading: isProfileLoading } =
    useCollection<UserProfile>(userQuery);
