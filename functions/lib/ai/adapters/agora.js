 ...
 */
export function createUserToken(uid, channelName, expireSeconds = 3600) {
    ...
    return {
        appId: AGORA_APP_ID,
        token: RtcTokenBuilder.buildTokenWithAccount(AGORA_APP_ID, AGORA_APP_CERT, channelName, uid, role, expireTs),
        channelName,
    };
}
/**
 ...
 */
export function createAIToken(channelName, expireSeconds = 3600) {
    if (!AGORA_APP_ID || !AGORA_APP_CERT) {
        throw new Error("Missing AGORA_APP_ID or AGORA_APP_CERT");
    }
    ...
    return {
        appId: AGORA_APP_ID,
        token: RtcTokenBuilder.buildTokenWithAccount(AGORA_APP_ID, AGORA_APP_CERT, channelName, AI_UID, role, expireTs),
        channelName,
        aiUid: AI_UID,
    };
}
 ...
 */
export async function publishAudioToAgora(channelName, audioBuffer) {
