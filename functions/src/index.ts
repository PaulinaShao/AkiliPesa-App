 ...
 ***********************************************/
export const createVoiceCloneV2 = functions.https.onRequest(async (req, res) => {
  try {
...
/***********************************************
 * aiRouter2 (Gen-1 HTTP)
 * Handles chat requests to AI / Agents
 ***********************************************/
export const aiRouter2 = functions.https.onRequest(async (req, res) => {
  try {
...
/***********************************************
 * callSessionHandler2 (Gen-1 HTTP)
 * Creates or updates a call session in Firestore.
 ***********************************************/
export const callSessionHandler2 = functions.https.onRequest(async (req, res) => {
  try {
...
/***********************************************
 * schedulePublisher2 (Gen-1 Scheduled)
 * Publishes posts that were scheduled future.
 ***********************************************/
export const schedulePublisher2 = functions.pubsub
...
/***********************************************
 * vendorOptimizer2 (Gen-1 Scheduled)
 * Recalculates vendor performance metrics daily.
 ***********************************************/
export const vendorOptimizer2 = functions.pubsub
...
/***********************************************
 * callLiveLoop2 (Gen-1 HTTP)
 * Used by front-end every 20-40s to keep call alive.
 ***********************************************/
export const callLiveLoop2 = functions.https.onRequest(async (req, res) => {
  try {
    const { callId, userId } = req.body;
...
/***********************************************
 * socialPoster2 (Gen-1 HTTP)
 * Posts automated or AI-generated content.
 ***********************************************/
export const socialPoster2 = functions.https.onRequest(async (req, res) => {
  try {
...
export const onVoiceUpload = functions.storage.object().onFinalize(async (object) => {
  try {
    const filePath = object.name || "";
    if (!filePath.startsWith("voice-uploads/")) return; // only react to our folder
