...
export const createAiCallSession = onCall({ region: "us-central1" }, async (request) => {
    const auth = request.auth;
    if (!auth) {
        throw new HttpsError("unauthenticated", "Sign-in required.");
    }
    const { persona = "empathetic", language = "sw", agentId = "akili-ai", callType = "audio", // "audio" | "video"
